...
def plotDecBoundaries_Nonlinear(feature, labels, non_linear_trans, predictor, order, fsize=(6,4),legend_on = False):
    ...
    xy = non_linear_trans(xy, order)
    # predict the class of all points in the map 
    pred_label = predictor(np.concatenate([np.ones(len(xy)).reshape(len(xy), 1), xy], axis=1))
