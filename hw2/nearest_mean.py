import numpy as np
from utils.plotDecBoundaries_2 import plotDecBoundaries_2
...
if __name__ == '__main__':
    test_path = './HW2_datasets/HW2_test_set.csv'
    train_path = './HW2_datasets/HW2_train_set.csv'

    #change train_path and test_path
    orig_data, fea_map, labels, labels_type, fea_num, label_num, data_num, num_data_per_class, classes_data_list = data_info(train_path)
    orig_data_test, fea_map_test, labels_test, labels_type_test, fea_num_test, label_num_test, data_num_test, num_data_per_class_test, classes_data_list_test = data_info(test_path)
    
    #Q(a)
    cl_mean = class_mean(label_num, classes_data_list, fea_num)
    print(cl_mean)
    #print(fea_map.shape)
    #plotDecBoundaries_2(fea_map, labels, cl_mean)
    #plotDecBoundaries_2(fea_map_test, labels_test, cl_mean)
    ...
    print('Acc for training set:', 1-err_rate)
    print('Acc for testing set:', 1-err_rate_test)
    
    #(b)(c)(d)
    fea_map = fea_map[:, [2, 6]]
    fea_num = fea_map.shape[1]
    fea_map_test = fea_map_test[:, [2, 6]]
    fea_num_test = fea_map_test.shape[1]
    cl_mean_sub = cl_mean[:, [2, 6]]
    #print(fea_map.shape)
    #print(labels.astype(int))
    labels_plot = labels.copy()

    plotDecBoundaries_2(fea_map, labels_plot.astype(int), cl_mean_sub)
    #plotDecBoundaries_2(fea_map_test, labels_test, cl_mean)
    err_rate, eval = validate(fea_map, labels, cl_mean_sub, labels_type, fea_num)
    err_rate_test, eval_test = validate(fea_map_test, labels_test, cl_mean_sub, labels_type_test, fea_num_test)
    print('Error rate for training set:', err_rate)
    print('Error rate for testing set:', err_rate_test)
    print('Acc for training set:', 1 - err_rate)
    print('Acc for testing set:', 1 - err_rate_test)
